        ...
        <el-aside width="200px">
          <el-menu :default-openeds="['1', '3']" v-if="titlenav!=6">
            <el-menu-item
              ...
              :key="i"
              :index="item.index"
              @click="addTab(item.index,item.title,item.url)"
            >
            ...
            <el-submenu :index="item.id" v-for="item in menu" :key="item.id">
              ...
              <el-menu-item-group>
                <el-menu-item
                  ...
                  :key="i.id"
                  @click="addTab(i.index,i.title,i.url)"
                >
      ...
      if (this.titlenav == 6) {
        for (let key of this.menu) {
          for (let i = 0; i <= this.menu.length; i++) {
            if (this.menu[i]) {
              if (this.menu[i].obj.list[i].index == obj.name) {
                this.$router.push(this.menu[i].obj.list[i].url);
              }
            }
          }
        }
      }
      for (let key of this.arrList) {
        if (obj.name == key.index) {
